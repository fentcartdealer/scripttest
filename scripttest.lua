...
library.title = "XSX VIGIL"

library:Introduction()
wait(1)
local Init = library:Init()

local Tab1 = Init:NewTab("CamLock")
local Tab2 = Init:NewTab("SilentAim")
local Tab3 = Init:NewTab("Esp")
local Tab4 = Init:NewTab("Discord")

local Section1 = Tab1:NewSection("CamLock")
local Section2 = Tab2:NewSection("SilentAim")
local Section3 = Tab3:NewSection("Esp")
local Section3 = Tab4:NewSection("Discord")

local Label1 = Tab1:NewLabel("Cam Lock", "left")
local Label2 = Tab2:NewLabel("Silent Aim", "left")
local Label3 = Tab3:NewLabel("ESP (Box Outline)", "left")
local Label4 = Tab3:NewLabel("Highlight ESP")

local Toggle3 = Tab1:NewToggle("Enable", false, function(value)
...
local Toggle3 = Tab2:NewToggle("Enable", false, function(value)
    local vers = value and "on" or "off"
    print("one " .. vers)
end):AddKeybind(Enum.KeyCode.Q)

local Toggle2 = Tab2:NewToggle("Toggle", false, function(value)
    local vers = value and "on" or "off"
    print("one " .. vers)
end):AddKeybind(Enum.KeyCode.W)

local Button1 = Tab2:NewButton("Button", function()
    print("one")
end)

local Keybind1 = Tab2:NewKeybind("Keybind 1", Enum.KeyCode.RightAlt, function(key)
    Init:UpdateKeybind(Enum.KeyCode[key])
end)

local Textbox1 = Tab2:NewTextbox("Text box 1 [auto scales // small]", "", "1", "all", "small", true, false, function(val)
    print(val)
end)

local Textbox2 = Tab2:NewTextbox("Text box 2 [medium]", "", "2", "all", "medium", true, false, function(val)
    print(val)
end)

local Textbox3 = Tab2:NewTextbox("Text box 3 [large]", "", "3", "all", "large", true, false, function(val)
    print(val)
end)

local Selector1 = Tab2:NewSelector("Selector 1", "bungie", {"fg", "fge", "fg", "fg"}, function(d)
    print(d)
end):AddOption("fge")

local Slider1 = Tab2:NewSlider("Slider 1", "", true, "/", {min = 1, max = 100, default = 20}, function(value)
    print(value)
end)
